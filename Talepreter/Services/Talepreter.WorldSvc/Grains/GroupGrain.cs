...
[GenerateSerializer]
public class GroupGrain : CommandGrain, IGroupGrain
{
    public GroupGrain(ILogger<GroupGrain> logger, IDocumentDbContext documentDbContext) : base(logger, documentDbContext) { }

    protected override async Task ExecuteCommandAsync(ExecuteCommandContext commandInfo, CancellationToken token)
    {
        if (commandInfo.Command.Tag != Model.Command.CommandIds.Group)
            throw new CommandExecutionException(commandInfo.Command.ToString()!, "Command is not recognized for execution");

        var commandExecutor = _scope.ServiceProvider.GetRequiredService<ICommandExecutor<IGroupGrain>>() ?? throw new CommandExecutionException($"Registration of {typeof(IGroupGrain).Name} command executor is invalid");
